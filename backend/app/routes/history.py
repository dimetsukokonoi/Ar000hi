...
# PDF receipt
# ---------------------------------------------------------------------------
# Amounts are written as "BDT 100.00", never the Taka sign. fpdf2's built-in
# fonts are Latin-1 only, and U+09F3 raises FPDFUnicodeEncodingException; the
# alternative is embedding a ~700KB Unicode TTF for one glyph.
...
def _build_pdf(r: dict) -> bytes:
    ...
    pdf.cell(W * 0.5, 5, "Issued " + _fmt_dt(r["issued_at"]), align="R",
             new_x="LMARGIN", new_y="NEXT")
    pdf.set_text_color(0)
    rule()

    status = "PAID" if r["fully_paid"] else ("PARTIALLY SETTLED" if r["paid"] else "UNSETTLED")
    row("You " + ("drove this ride" if r["role"] == "driver" else "rode as a passenger"),
        status, bold=True)

    heading("Trip")
    row("Route", f'{r["route"]["source"]} to {r["route"]["destination"]}')
    if r["role"] == "passenger":
        row("Your pickup / drop-off",
    ...
    for b in r["breakdown"]:
        row(f'{b["name"]}  ({b["seats"]} seat{"" if b["seats"] == 1 else "s"})',
            _money(b["share"]))

    rule()
    row(r["your_line_label"], _money(r["your_amount"] if r["paid"] else r["expected_amount"]),
        bold=True, size=12)
    ...
    if not r["fully_paid"]:
        ...
        note = (f'Partially settled: {_money(r["expected_amount"])} was owed, '
                f'{_money(r["shortfall"])} was never received.'
                if r["paid"] else
                "This fare was never settled - the amount above is what was owed.")
    ...
    if r["transaction_id"]:
        ...
        pdf.cell(W, 5, "Transaction " + str(r["transaction_id"]),
                 new_x="LMARGIN", new_y="NEXT")
        pdf.set_text_color(0)
    ...
    footer = "Thank you for riding with Arooohi." + chr(10) + "Demo system - no real currency was transferred."
...
def _fmt_dt(value) -> str:
    ...
    if naive is None:
        return str(value)
    return naive.replace(tzinfo=ZoneInfo("UTC")).astimezone(BD_TZ).strftime("%d %b %Y, %H:%M")
...
def get_receipt_pdf(ride_id: str, user_id: str = Depends(get_current_user_id)):
    ...
    data = get_receipt(ride_id, user_id)      # reuses every guard and calculation
    pdf = _build_pdf(data)
