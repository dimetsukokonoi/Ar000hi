...
Feature 3: Live GPS Ride Tracking
"""
import uuid
...
def start_tracking_session(user_id: str = Depends(get_current_user_id)):
    ...
    share_token = uuid.uuid4().hex[:12]  # Short shareable token
...
def get_session_points(session_id: str, user_id: str = Depends(get_current_user_id)):
    """Get all tracking points for a session (authenticated)."""
    conn = get_db()
    points = conn.execute(
...
def stop_tracking_session(session_id: str, user_id: str = Depends(get_current_user_id)):
    ...
    conn = get_db()
    conn.execute(
        "UPDATE tracking_sessions SET is_active = 0 WHERE id = ? AND user_id = ?",
        (session_id, user_id)
    )
    conn.commit()
    conn.close()
