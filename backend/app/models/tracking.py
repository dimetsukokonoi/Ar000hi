"""Tracking model: business rules and persistence, independent of FastAPI."""
import uuid
from app.models.database import get_db
from app.schemas.tracking import TrackingPointRequest, StartSessionRequest
from app.models.errors import DomainError


def start_tracking_session(user_id: str):
...
def add_tracking_point(body: TrackingPointRequest, user_id: str):
    ...
    if not session:
        conn.close()
        raise DomainError(status_code=404, detail="Active tracking session not found")
...
def get_session_points(session_id: str, user_id: str):
    ...
    if not session:
        conn.close()
        raise DomainError(status_code=404, detail="Tracking session not found")
...
def stop_tracking_session(session_id: str, user_id: str):
