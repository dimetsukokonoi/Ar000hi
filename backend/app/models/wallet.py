"""Wallet model: business rules and persistence, independent of FastAPI."""
import os
import uuid
...
def start_topup(body: TopUpRequest, user_id: str):
    ...
    try:
        ...
        raise DomainError(status_code=502, detail=f"Payment gateway unavailable: {e}")
...
def execute_topup(body: ExecuteRequest, user_id: str):
    ...
    if not payment:
        conn.close()
        raise DomainError(status_code=404, detail="Payment not found")
    ...
    try:
        ...
        conn.close()
        raise DomainError(status_code=500, detail="Could not credit the wallet")
...
def payment_status(payment_id: str, user_id: str):
...
def withdraw(body: WithdrawRequest, user_id: str):
    """Cash out to bKash — the other edge of the prepaid model."""
    amount = round(body.amount, 2)
    if amount <= 0:
        raise DomainError(status_code=400, detail="Withdrawal amount must be positive")
    ...
    try:
        ...
        conn.close()
        raise DomainError(status_code=402, detail=str(e))
